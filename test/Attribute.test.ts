...
import { NumericAttribute as NumericConfig, BooleanAttribute as BooleanConfig } from "./test.config";
...
describe('Numeric', () => {
    ...
    describe('Mutation', () => {
        ...
        test('Should replace the value on mutation', () => {
            let param = new NumericAttribute(NumericConfig({
                mutation: {
                    prob: {
                        offset: 0,
                        replace: 1
    ...
    describe('Clone', () => {
        ...
        test('Clone should not reference original param', () => {
            let param = new NumericAttribute(NumericConfig());
            let clone = param.Clone();
            expect(clone).not.toBe(param);
        })

        test('Cloned value should equal original value', () => {
            let param = new NumericAttribute(NumericConfig());
            let clone = param.Clone();
            expect(clone.value).toEqual(param.value);
        })
    })
})
...
describe('Boolean', () => {
    ...
    describe('Mutation', () => {
        ...
        test('Should flip the value on mutation', () => {
            let param = new BooleanAttribute(BooleanConfig({
                mutation: {
                    prob: 1
                }
            }));
        ...
        test('Should run mutation according to probability', () => {
            let param = new BooleanAttribute(BooleanConfig({
                mutation: {
                    prob: 0.2
                }
